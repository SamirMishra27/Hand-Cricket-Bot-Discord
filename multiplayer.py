...
HandCricket will be hosting a tournament very soon! To participate, join the [**support server**](https://discord.gg/Mb9s5sVkSz ) Here!

Let your friends know as well of this news.
For more information, see h!news.

**(Registration ends on 15 June 5:30pm UTC / 16 June 12:00am IST)**"""

    @commands.command(
        aliases = ['c', 'create_multiplayer', 'multiplayer'],
        brief = 'Creates a multiplayer game.',
        usage = [f"h! create | c | create_multiplayer | multiplayer", None],
    ...
    async def _start_multiplayer_match(self, ctx):
        ...
        def __check_for_hattrick(player, timeline):
        ...
        while True:
            ...
            if game.curr_batsman['last_action'] != game.curr_bowler['last_action']: 
                ...
                _hattrick = __check_for_hattrick(game.curr_bowler['player'].name, game.curr_bowler['timeline'])
                if _hattrick == True:
                    game.curr_bowler['hattrick'] = True
                
                await game.send_msg('BATSMAN', "You are {}out! \nOpponent's last action -> {} \n".format('DUCK ' if game.curr_batsman['runs'] == 0 else '', game.curr_bowler['last_action']) + 
                                    "score: {runs} ({balls})".format(**game.curr_batsman) + f" {ctx.channel.mention}")

                await game.send_msg('BOWLER', "Opponent is {}out! ".format('DUCK ' if game.curr_batsman['runs'] == 0 else '') +
                                    "\nOpponent's last action -> {last_action} \nscore: {runs} ({balls})".format(**game.curr_batsman))

                msg = '\n'.join([
                    "**{}** is {} bowled by **{}**".format(game.curr_batsman['player'].name, 'DUCK out! :joy: :duck:' if game.curr_batsman['runs'] == 0 else 'out!', game.curr_bowler['player'].name),
                ...
                for playerid in game.batting_team.keys():
                    if playerid in ('name','runs','balls','wickets') or game.batting_team[playerid]['is_out'] == True:
                        ...
                        next_batsman_msg2 = f"Next batsman is **{game.curr_batsman['player'].name}**"
                        await game.send_msg('BOWLER', next_batsman_msg2)
                    ...
                    await ctx.send(msg)
                    await game.send_msg('BOWLER', f"**Their team** is all out! Innings over. {ctx.channel.mention}")
                    await game.send_msg('BATSMAN', f"**Your team** is all out! Innings over. {ctx.channel.mention}")
                    break
            ...
            if game.batting_team['balls'] == game.overs:
                ...
                await ctx.send(msg)
                await game.send_msg('BATSMAN', f"Next bowler to bowl is {game.curr_bowler['player'].name}")
                participants_ids = [game.curr_batsman['player'].id, game.curr_bowler['player'].id]
                continue
        ...
        while True:
            ...
            if game.curr_batsman['last_action'] != game.curr_bowler['last_action']: 
                ...
                if game.batting_team['runs'] > game.bowling_team['runs']:
                    msg = "Score has been chased! {}".format(ctx.channel.mention)
                    await game.send_msg('BATSMAN', msg)
                    await game.send_msg('BOWLER', msg)
                ...
                msg = '\n'.join([
                    "**{}** is {} bowled by **{}**".format(game.curr_batsman['player'].name, 'DUCK! :joy: :duck:' if game.curr_batsman['runs'] == 0 else 'out!', game.curr_bowler['player'].name),
                    "```{player.name}: {runs} ({balls})".format(**game.curr_batsman),
                    "{player.name}: {wickets} - {runs_given}```".format(**game.curr_bowler),
                    "**{name}** score: {runs}/{wickets}".format(**game.batting_team) + \
                    "  -  {} overs\n\n".format(overs(game.batting_team['balls']))
        ...
        try: 
            await game.generate_scorecard_image(footer = self.match_end_footer)
    ...
    @commands.command(
        brief = 'Shows the current status of the game.', 
        usage = ['h! status', None], 
    ...
    async def status(self, ctx):        
        ...
        if game.is_updating == True:
            last_timestamp = self.bot.last_message_cache.get(ctx.guild.id, 0)
            if time.time() - last_timestamp < 1.2:
                return
    ...
    @commands.command(
        ...
        usage = ['h! changeteamname | editteamname | teamname <Team name> <Team\'s new name>', ['h!changeteamname Teama Champions', 'h!teamname teamb Warriors']],
